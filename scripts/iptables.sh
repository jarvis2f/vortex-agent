...
install_netstat() {
  netstat -V >/dev/null 2>&1 && return 0
  if [[ $OS_FAMILY == "centos" ]]; then
    $INSTALL net-tools || ($UPDATE && $INSTALL net-tools) || (echo "Failed to install net-tools" && exit 1)
  elif [[ $OS_FAMILY == "debian" ]]; then
    $INSTALL net-tools || ($UPDATE && $INSTALL net-tools) || (echo "Failed to install net-tools" && exit 1)
  else
    echo "netstat command not found" && exit 1
  fi
}
...
if [[ ! $TYPE == "ALL" && ! $TYPE == "TCP" && ! $TYPE == "UDP" ]]; then
